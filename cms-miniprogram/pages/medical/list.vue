<template>
  <view class="medical-list-container">
    <!-- 头部导航 -->
    <view class="header" :style="{ paddingTop: statusBarHeight + 'px' }">
      ...
      <view class="header-title">医疗管理</view>
    ...
    <view class="content-container">
      ...
      <!-- 空状态 -->
      <view v-else-if="medicalList.length === 0" class="empty-container">
        <view class="empty-icon">
          <uni-icons type="medkit" size="64" color="#ccc"></uni-icons>
        </view>
        <!-- 区分搜索结果为空和本来就没有数据的情况 -->
        <text v-if="searchKeyword" class="empty-title">找不到该医疗记录</text>
        <text v-else class="empty-title">暂无医疗记录</text>
        
        <text v-if="searchKeyword" class="empty-subtitle">没有找到匹配的医疗记录，请尝试其他关键词</text>
        <text v-else class="empty-subtitle">您可以点击右上角添加医疗记录</text>
      ...
      <view v-else class="list-container">
        <uni-list>
          ...
          >
            <template #header>
              <view class="medical-header" @click="handleDetail(item)">
                <text class="medical-name">{{ item.residentName }}</text>
              </view>
            </template>
            <template #footer>
              <view class="medical-footer">
                <view class="medical-actions">
...
export default {
  ...
  methods: {
    ...
    loadMedicalList() {
      ...
      // 调用医疗记录列表API
      get('/medical/list', params)
        .then(res => {
          let allMedicalList = []
          let total = 0
          
          if (res.code === 200 && res.data) {
            allMedicalList = res.data.data || []
      ...
      if (this.searchKeyword) {
        ...
        this.medicalList = allMedicalList.filter(item => {
          // 匹配姓名或医院名称
          return item.residentName?.includes(trimmedKeyword) || 
                 item.hospital?.includes(trimmedKeyword)
        })
        this.total = this.medicalList.length
      } else {
        this.medicalList = allMedicalList
        this.total = total
      }
        })
        .catch(err => {
          console.error('获取医疗记录列表失败', err)
          uni.showToast({
            title: '获取医疗记录失败',
            icon: 'none'
          })
    ...
    handleEdit(item) {
      uni.navigateTo({
        url: `/pages/medical/edit?id=${item.id}`
      })
    },
    ...
    handleDelete(item) {
      uni.showModal({
        title: '确认删除',
        content: `确定要删除医疗记录 ${item.id} 吗？`,
        success: (res) => {
          if (res.confirm) {
            // 调用删除API
            del(`/medical/delete/${item.id}`)
              .then(res => {
                if (res.code === 200) {
                  uni.showToast({
                    title: '删除成功',
    ...
    registerWebSocketListener() {
      this.dataChangeHandler = (message) => {
        console.log('医疗列表页面收到数据变更通知:', message)

        if (message.entityType === 'MEDICAL') {
