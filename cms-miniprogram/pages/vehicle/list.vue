<template>
  <view class="vehicle-list-container">
    ...
    <view class="header" :style="{ paddingTop: statusBarHeight + 'px' }">
      ...
      <view class="header-title">车辆管理</view>
    ...
    <view class="content-container">
      ...
      <!-- 空状态 -->
      <view v-else-if="vehicleList.length === 0" class="empty-container">
        <view class="empty-icon">
          <uni-icons type="car" size="64" color="#ccc"></uni-icons>
        </view>
        <!-- 区分搜索结果为空和本来就没有数据的情况 -->
        <text v-if="searchKeyword" class="empty-title">找不到该车辆记录</text>
        <text v-else class="empty-title">暂无车辆记录</text>
        
        <text v-if="searchKeyword" class="empty-subtitle">没有找到匹配的车辆记录，请尝试其他关键词</text>
        <text v-else class="empty-subtitle">您可以点击右上角添加车辆记录</text>
  ...
  methods: {
    ...
    loadVehicleList() {
      ...
      if (this.filters.vehicleType) {
        params.vehicleType = this.filters.vehicleType
      }
      
      // 调用车辆记录列表API
      get('/vehicle/list', params)
        .then(res => {
          let allVehicleList = []
          let total = 0
          ...
          if (res.code === 200 && res.data) {
            ...
            if (res.data.data) {
              ...
              allVehicleList = res.data.data || []
              total = res.data.total || 0
            } else if (res.data.records) {
              // 格式: { data: { records: [], total: 0 } }
              allVehicleList = res.data.records || []
              total = res.data.total || 0
            } else {
              // 格式: { data: [] }
              allVehicleList = Array.isArray(res.data) ? res.data : []
              total = allVehicleList.length
            }
          }
          ...
          if (this.searchKeyword) {
            ...
            this.vehicleList = allVehicleList.filter(item => {
              // 匹配姓名或车牌号
              return item.residentName?.includes(trimmedKeyword) || 
                     item.licensePlate?.includes(trimmedKeyword)
            })
            this.total = this.vehicleList.length
          } else {
            this.vehicleList = allVehicleList
            this.total = total
          }
        })
        .catch(err => {
          console.error('获取车辆记录列表失败', err)
          uni.showToast({
            title: '获取车辆记录失败',
            icon: 'none'
          })
    ...
    handleDetail(item) {
      uni.navigateTo({
        url: `/pages/vehicle/detail?id=${item.id}`
      })
    },
    ...
    handleEdit(item) {
      uni.navigateTo({
        url: `/pages/vehicle/edit?id=${item.id}`
      })
    },
    ...
    handleDelete(item) {
      uni.showModal({
        title: '确认删除',
        content: `确定要删除车辆记录 ${item.id} 吗？`,
        success: (res) => {
          if (res.confirm) {
            // 调用删除API
            del(`/vehicle/delete/${item.id}`)
              .then(res => {
                if (res.code === 200) {
                  uni.showToast({
                    title: '删除成功',
    ...
    registerWebSocketListener() {
      this.dataChangeHandler = (message) => {
        console.log('车辆列表页面收到数据变更通知:', message)

        if (message.entityType === 'VEHICLE') {
